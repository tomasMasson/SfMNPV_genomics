...
#    headers = [header[0] for header in dic['MLE']['headers']]
    ...
    df.columns = ['alpha', 'beta', 'Omega', 'alpha=beta',
                  'LRT', 'p-value', 'Total branch length']
    return df
...
def assign_selective_pressure(df):
    ...
    for index, row in df.iterrows():
        if row['p-value'] <= 0.1 and row['Omega'] < 1:
            selection_type.append('Purifiyng')
        elif row['p-value'] <= 0.1 and row['Omega'] > 1:
...
def argument_parser():
    ...
    parser.add_argument('alignment',
                        help='Alignment used to filter codons corresponding to SfMNPV')
